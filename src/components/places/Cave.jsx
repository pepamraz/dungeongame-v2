...
const caveImages = import.meta.glob('../../assets/caves/*.webp', { eager: true });
const zombieImages = import.meta.glob('../../assets/zombies/*.webp', { eager: true });
const diamondImages = import.meta.glob('../../assets/diamonds/*.webp', { eager: true });

const getCaveImage = (index) => caveImages[`../../assets/caves/cave-${index}.webp`]?.default || '';
const getZombieImage = (index) => zombieImages[`../../assets/zombies/zombie-${index}.webp`]?.default || '';
const getDiamondImage = (index) => diamondImages[`../../assets/diamonds/diamond-${index}.webp`]?.default || '';
    ...
    return (
        <>
            ...
            <div id="game-graphics" className="h-39/48 bg-gray-500 rounded-t-lg relative overflow-hidden">
                <div className="h-full" style={{ backgroundImage: `url(${getCaveImage(caveImage)})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                    <div className="w-full h-full bg-radial from-40% to-black opacity-80" />
                </div>
                {player.roomType === 'zombie' && (
                    <img src={getZombieImage(zombieSkin)} alt="zombie" className="max-h-4/5 drop-shadow-[0_30px_60px_rgba(0,0,0,0.75)] absolute bottom-[-15px] left-1/2 transform -translate-x-1/2" />
                )}
                {player.roomType === 'diamond' && (
                    <img src={getDiamondImage(player.diamondUpgrade)} alt="diamond" className="max-h-2/5 drop-shadow-[0_30px_60px_rgba(0,0,0,0.75)] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                )}
            </div>
